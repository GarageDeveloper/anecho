...
/// waveform in the consumer's signal-source model. The limits are now wired into the
/// auto-level procedure (in the consuming application); the dBV *values* below remain
...
impl InputGain {
    ...
    /// (`crate::discovery::DeviceCapabilities` reads it — issue #25 lot B).
