...
pub enum DeviceError {
    ...
    /// the consumer's device registry).
...
mod tests {
    ...
    fn not_found_displays_exactly_like_the_legacy_qa40x_variant() {
        ...
        assert_eq!(
            ...
            QA40xError::DeviceNotFound.to_string()
        );
    ...
    fn qa40x_errors_display_unchanged_through_the_seam() {
        ...
        let wrapped: DeviceError =
            QA40xError::DeviceError("Failed to open device: busy".into()).into();
        assert_eq!(wrapped.to_string(), e.to_string());
        // And DeviceNotFound folds into the dedicated variant.
        assert!(matches!(
            DeviceError::from(QA40xError::DeviceNotFound),
