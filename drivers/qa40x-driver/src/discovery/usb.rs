...
use crate::{Model, QA40X_VID, QA40xError};
...
mod tests {
    use super::*;
    #[cfg(feature = "sim")]
    use crate::QA40xDevice;
    #[cfg(feature = "sim")]
    use crate::transport::demo_sim_options;
    use crate::{QA402_PID, QA403_PID};
    #[cfg(feature = "sim")]
    use std::sync::Arc;
    #[cfg(feature = "sim")]
    ...
    fn colliding_serials_are_disambiguated_with_the_bus_path_not_collapsed() {
        ...
        let units = vec![
            unit(Some("AB12_CD34"), 1),
            unit(Some("AB12_CD34"), 2),
            unit(Some("EE00_0001"), 3),
        ];
    ...
    fn matches_port_requires_the_exact_bus_position() {
        ...
        assert!(
            !matches_port(&u, "21", &[2, 1]),
            "a different bus is a different position"
        );
        assert!(
            !matches_port(&u, "20", &[2, 2]),
            "a different port is a different position"
        );
        assert!(
            !matches_port(&u, "20", &[2]),
            "a chain prefix is not the position"
        );
    }
    ...
    fn descriptor_is_built_purely_from_the_unit() {
        ...
        assert_eq!(
            d.identity.firmware_version, None,
            "unopened: no firmware yet"
        );
    ...
    async fn open_releases_the_prior_claim_before_scanning_for_the_unit() {
        ...
        let err = src
            .open(&impossible, &handle)
            .await
            .expect_err("no such unit on the bus");
