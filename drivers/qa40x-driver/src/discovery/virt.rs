...
impl VirtualDeviceSource {
    ...
    /// The units this source hosts, in enumeration order.
...
impl DeviceSource for VirtualDeviceSource {
    ...
    async fn open(
        ...
        dev.connect_virtual_sim(sim, unit.model())
            .await
...
mod tests {
    ...
    async fn enumeration_does_not_instantiate_the_simulator() {
        ...
        let descs = src
            .enumerate()
            .await
            .expect("virtual enumerate is infallible");
        ...
        assert_eq!(
            d.id.as_str(),
            format!("virtual/{}", demo_unit_options(0).serial)
        );
        assert_eq!(
            descs[1].id.as_str(),
            format!("virtual/{}", demo_unit_options(1).serial)
        );
        for d in &descs {
