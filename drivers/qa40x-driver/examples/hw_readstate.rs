...
async fn main() {
    ...
    if let Err(e) = device.connect().await {
        ...
        eprintln!(
            "If the desktop app is open it holds the single connection — close it and retry."
        );
        std::process::exit(1);
    }
    ...
    match device.read_input_gain().await {
        Ok(g) => println!(
            "  INPUT gain (reg 5)  = {} dBV  (register value {})",
            g.as_dbv(),
            g.as_register_value()
        ),
    ...
    match device.read_output_gain().await {
        Ok(g) => println!(
            "  OUTPUT gain (reg 6) = {} dBV  (register value {})",
            g.as_dbv(),
            g.as_register_value()
        ),
